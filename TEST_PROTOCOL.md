...
### `js/grist-api.js` — course au démarrage entre `grist.ready()` et le 1er `docApi` (`table-race-test.js`, Playwright, mock étendu avec `__gristStubRaceCalls`/`__gristStubPreseed`)

- [x] [BUG RÉEL #18 ci-dessus] Session sans course simulée : création normale d'une table qui n'existe pas encore, `created: true`, volume exact 🌐
- [x] [BUG RÉEL #18 ci-dessus] Session avec course simulée sur le 1er appel `listTables()` d'une table qui existe DÉJÀ (préremplie via `__gristStubPreseed`, simule une réouverture du widget) : `tableExistsConfirmed` rattrape le faux négatif, `created: false`, AUCUNE duplication de lignes 🌐 — vérifié en confirmant d'abord que ce même test ÉCHOUE sans le correctif (`created: true`, régression volontairement reproduite en isolant `js/grist-api.js` à son état pré-correctif via `git stash`), preuve que le test est bien significatif
- [x] Sans course réelle, le rattrapage (délai de 500ms) ne se déclenche QU'UNE SEULE FOIS par session (pas à chaque contrôle d'existence — `BI_StressTest` puis `BI_Dashboard_Config`) : les deux contrôles combinés restent sous 1000ms 🌐
